                        ...
                        val listener = object : ValueEventListener {
                            ...
                            override fun onDataChange(chatSnapshot: DataSnapshot) {
                                if (chatSnapshot.exists()) {
                                    ...
                                    if (lastMsgObj != null && lastMsgObj.senderId != uid) {
                                        val now = System.currentTimeMillis()
                                        // Only notify if message is less than 10 seconds old and it's not the first load
                                        if (!firstLoad && (now - lastMsgObj.timestamp) < 10000) {
